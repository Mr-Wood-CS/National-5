...
Each table needs a __primary key__ – a unique ID to identify each record. The Loan table will also need __foreign keys__ to link to the Book and Pupil tables.
...
==__The database must also be able to:__==
...
!!! summary "Summary"
    ...
    During analysis, you identify who will use the database and what they need it to do. End-user requirements describe the tasks users want to complete, while functional requirements describe the tables, fields, keys, and actions the database must include.
